...
coxRegression <- function(survivalTime, categoricalInput, continuousInput, statusVariable, status, displayDescriptives = TRUE,
                          displayCoefficientEstimates = TRUE, displayModelFit = TRUE, hazardRatio = TRUE, goodnessOfFitTests = TRUE, analysisOfDeviance = TRUE,
                          ...
                          storeSchoenfeldResiduals = FALSE, storeDfBetas = FALSE, CoxPH = TRUE, CoxPhPlot = TRUE,  data = dataSet){
  ...
  if(modelSelection == "enter"){
    formula = as.formula(paste0("Surv(survivalTime, statusVar) ~ ", predictors))
    current = coxph(formula, data=newData, ties = ties)
  }
  
  if(modelSelection == "forward"){
    formula = as.formula(paste0("Surv(survivalTime, statusVar) ~ ",1))
    fullFormula = as.formula(paste0("Surv(survivalTime, statusVar) ~ ", predictors))
    ...
    if(modelSelectionCriteria == "aic"){
      ...
      while (TRUE) {
        formulaFull = as.formula(paste0("Surv(survivalTime, statusVar) ~ ", predictors))
  ...
  if(modelSelection == "stepwise"){
    ...
    formula = paste0("Surv(survivalTime, statusVar) ~ ", 1)
    coeffs = length(current$coefficients)
    if(modelSelectionCriteria == "aic"){
